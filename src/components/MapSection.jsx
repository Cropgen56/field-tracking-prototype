...
import { MapContainer, TileLayer, Polygon, useMap, GeoJSON, Marker, useMapEvents } from 'react-leaflet';
...
export default function MapSection({ uploadedData, externalLocation, onLocationChange, onFieldSave, selectedSnapshot }) {
  ...
  useEffect(() => {
    if (selectedSnapshot && selectedSnapshot.markers && selectedSnapshot.id !== lastSnapshotIdRef.current) {
      console.log('Loading snapshot:', selectedSnapshot.name, selectedSnapshot.id);
  ...
  const handleGenerateField = () => {
    ...
    setTimeout(() => {
      ...
      showToast(`${sampleFieldsData.features.length} sample fields loaded! Total area: ${totalArea.toFixed(2)} ha`, 'success');
    }, 1000);
  };
  ...
  const handleSaveBoundary = () => {
    ...
    try {
      const existingSnapshots = JSON.parse(localStorage.getItem('fieldSnapshots') || '[]');
      ...
      showToast(`Field saved! Area: ${manualArea.toFixed(2)} hectares`, 'success');
  ...
  const polygonBounds = useMemo(() => {
    if (sampleFields && sampleFields.features.length > 0) {
      const allCoords = [];
      sampleFields.features.forEach(feature => {
        if (feature.geometry.type === 'Polygon') {
          feature.geometry.coordinates[0].forEach(coord => {
            allCoords.push({ lng: coord[0], lat: coord[1] });
          });
    ...
    if (generatedPolygon.length === 0 && manualMarkers.length === 0) return null;
  ...
  }, [generatedPolygon, manualMarkers, sampleFields]);
    ...
    <>
      <div className="bg-cg-panel rounded-xl sm:rounded-2xl shadow-cg-soft h-[400px] sm:h-[450px] md:h-[500px] relative overflow-hidden border border-green-900/20">
        ...
        >
          ...
          {sampleFields && sampleFields.features && (
            <GeoJSON
              key="sample-fields"
              data={sampleFields}
              style={{
          ...
          {generatedPolygon.length > 0 && !uploadedData?.geojson && manualMarkers.length === 0 && !sampleFields && (
            <Polygon
        ...
        <MapOverlays
          ...
          sampleFieldsCount={sampleFields?.features?.length || 0}
          toast={toast}
